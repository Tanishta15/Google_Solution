...
def search_similar_content(text):
    try:
        ...
        response = model.generate_content(f"Find sources or similar articles related to: {text}. Provide URLs if possible.")
...
def check_plagiarism(original_text, web_texts):
    try:
        ...
        similarity_scores = similarity_matrix[0][1:]  # Compare original text with others
...
def process_content(content):
    try:
        ...
        web_texts = [extract_text_from_url(link) for link in matching_links if link.startswith("http")]
        ...
        logger.info(f"Plagiarism check result: {flagged} | Similarity Scores: {similarity_scores}")
